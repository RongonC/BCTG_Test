...
namespace StemmonsMobile.Views.Standards.TreeView
{
    public partial class AdaptTreeViewItemProvider : IEnumerable<TreeViewItem>, INotifyCollectionChanged
    {
        ...
        private void Refresh()
        {
            ...
            {
                _TreeViewItems.Add(CreateTreeViewItem(child, RootNodeText));
            }
            FinishRefresh();
        ...
        private TreeViewItem CreateTreeViewItem(object child, string propertyName)
        {
            ...
            {
                treeNode.IsExpanded = IsAllNodesExpanded;
            }
            else
            ...
            foreach (var childPropertyName in _ProbingPaths)
            {
                ...
                var childProperty = child.GetType().GetRuntimeProperty(childPropertyName);
                ...
                var childEnumerable = childValue as IEnumerable;
                if (childEnumerable != null && !childEnumerable.GetEnumerator().MoveNext())
                {
                    continue;
                }
                ...
                {
                    childEnumerable = new[] { childValue };
                }
                foreach (var nestedChild in childEnumerable)
                {
                    var treeViewItem = CreateTreeViewItem(nestedChild, childProperty.Name);
